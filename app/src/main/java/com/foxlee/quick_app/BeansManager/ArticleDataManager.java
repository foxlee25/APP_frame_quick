package com.foxlee.quick_app.BeansManager;

import com.foxlee.quick_app.entity.ArticleBean;
import com.foxlee.quick_app.entity.AuthorBean;
import com.foxlee.quick_app.entity.TagBean;
import com.foxlee.quick_app.utils.CTextUtils;
import com.foxlee.quick_app.utils.HttpRequest;
import com.foxlee.quick_app.utils.ImageUtils;
...
public class ArticleDataManager {
    ...
    public ArticleBean getArticleBean(Document document){
    	...
    	//获取头图片
    	String headImage= ImageUtils
                .getCutImageUrl(singleElement.select("div.single-post-header__headline").first().select("img[src]").first().attr("src"));
    	//获取文章内容
        ...
        //开始抓取用户信息
	    String author_Str= HttpRequest
                    .sendPost("http://36kr.com/asynces/posts/author_info", "url_code="+articleId);
